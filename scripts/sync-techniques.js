...
async function syncTechniques() {
    try {
        ...
        if (batchMachines.length === 0) {
            console.log(`⚠️  No machines in this batch. Batch number might be too high.`);
            console.log(`   Use: node scripts/sync-techniques.js [batch_number]`);
            console.log(`   Example: node scripts/sync-techniques.js 1`);
            return;
        }
        ...
        for (const machine of batchMachines) {
            try {
                ...
                if (tagsData && tagsData.info && Array.isArray(tagsData.info)) {
                    ...
                    console.log(`  ℹ️  No tags data returned`);
                }
        ...
        if (relationships.length > 0) {
            // Delete existing relationships for these machines
        ...
        if (batchNumber < totalBatches) {
            console.log(`\n📌 Next batch: node scripts/sync-techniques.js ${batchNumber + 1}`);
        } else {
